...
// A simple windows bitmap transformer module
...
function Transformer(bitmap) {
  ...
  this.writeFile = function(fileName, palette) {
    ...
    palette = palette === undefined ? colorPalette : palette;

    var newFile = new Buffer(bitmap.length);
    bitmap.copy(newFile, 0, 0);
    Array.prototype.forEach.call(palette, function(byte, index) {
  ...
  this.getPalette = function() {
    var thisPalette = new Buffer(colorPalette.length);
    colorPalette.copy(thisPalette);
    return thisPalette;
  };
